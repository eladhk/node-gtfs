...
const downloadFiles = async task => {
  ...
  if (/^ftp:\/\//.test(task.agency_url)){
      await ftpFetch(task.agency_url,task.path);
  }
  else{
      await httpFetch(task.agency_url,task.path);
  }
  task.log('Download successful');
};

const httpFetch = async (url,dstPath) =>{
...
const ftpFetch = (url,dstPath) =>{
    return new Promise((resolve,reject) => {
        const urlParse = new URL(url);
        const ftp = new jsftp({host: urlParse.host});
        ftp.on('error', err => {
            reject(new Error(`Couldn't download files - ${err.message}`));
        });
        ftp.get(urlParse.pathname, dstPath, err => {
            if (err) {
                reject(new Error(`Couldn't download files - ${err.message}`));
            }
            resolve();
